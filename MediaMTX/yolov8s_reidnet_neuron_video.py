...
def main():
    ...
    if os.path.exists(REID_MODEL):
        try:
            ...
            # onnxruntime defaults to spawning one intra-op thread per CPU
            # core, which competes for the same cores the decode/cairooverlay
            # display branch needs to stay in sync with the video's PTS
            # timeline -- that contention is what showed up as playback lag
            # once Re-ID inference was added on CPU. Capping it to 2 threads
            # leaves the rest of the cores free for GStreamer's own threads.
