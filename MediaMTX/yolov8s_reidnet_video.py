...
# Matches video1.avi's actual resolution (mpeg4/avi, 640x480) -- decodebin
# picks whatever software decoder handles that codec, and the BGRx capsfilter
# right after it just needs to agree with the source instead of forcing a
# resize.
VIDEO_WIDTH = 640
VIDEO_HEIGHT = 480

# Display-only upscale so the waylandsink window matches
# yolov8s_reidnet_rtsp_client_webcam.py's size -- waylandsink has no window
# size of its own, it just opens at whatever resolution reaches it, and
# video1.avi's native 640x480 would otherwise open a much smaller window.
# The inference branch still crops from the native-resolution frame, so this
# doesn't affect detection/Re-ID at all.
DISPLAY_WIDTH = 1280
DISPLAY_HEIGHT = 720
...
# No NPU path exists for this model on this board (no ncc-onnx converter, no
# nnstreamer onnxruntime subplugin), and Re-ID needs one crop per detected
# person anyway -- a variable-count input nnstreamer's static tensor_filter
# shapes can't express. So it runs imperatively on CPU via onnxruntime,
# invoked from the YOLOv8 tensor_sink callback instead of being wired into
# the GStreamer graph.
...
class Detector:
    ...
    def _reid_embed(self, dets):
        ...
        through the Re-ID model one at a time, and return a track id per
...
def build_pipeline():
    # Decode side: local file instead of a live RTSP source. decodebin
    # figures out the demuxer/decoder for whatever's in the file (video1.avi
    # is mpeg4-in-avi) on its own, same as ffplay/vlc would.
    cmd = (
        ...
        # NOT leaky, unlike the other two branches: decode races far ahead of
        # real-time (this file decodes way faster than 30fps), so a leaky
        # queue here would get refilled with fresh buffers and drop the
        # older, correctly-paced ones before sync=true's turn to render them
        # ever comes up -- that's what made playback blast through the whole
        # file in ~6 seconds despite sync=true "working". A plain blocking
        # queue instead makes this branch (and, via tee, decodebin itself)
        # back-pressure to the sink's real-time consumption rate, which is
        # what actually paces playback correctly.
        f't_raw. ! queue max-size-buffers=10 ! '
        f'videoscale ! video/x-raw,width={DISPLAY_WIDTH},height={DISPLAY_HEIGHT} ! '
        f'cairooverlay name=overlay ! videoconvert ! '
        # sync=true qos=true: the display branch (decode + cairooverlay) is
        # cheap on its own (gstreamer_video.py plays this file smoothly with
        # no inference at all), so it can honor the file's real PTS timeline
        # and play at the correct speed. The inference branch (YOLOv8 +
        # Re-ID, both synchronous inside on_new_data) is on its own tee
        # branch with its own leaky queue, so however slow it runs, it just
        # drops stale frames and never back-pressures this branch.
...
def main():
    ...
    if os.path.exists(REID_MODEL):
        # onnxruntime defaults to spawning one intra-op thread per CPU core,
        # which competes for the same cores the decode/cairooverlay display
        # branch needs to stay in sync with the video's PTS timeline -- that
        # contention is what showed up as playback lag once Re-ID inference
        # was added. Capping it to 2 threads leaves the rest of the cores
        # free for GStreamer's own threads.
        so = ort.SessionOptions()
        so.intra_op_num_threads = 2
        so.inter_op_num_threads = 1
        reid_session = ort.InferenceSession(REID_MODEL, sess_options=so, providers=['CPUExecutionProvider'])
    else:
