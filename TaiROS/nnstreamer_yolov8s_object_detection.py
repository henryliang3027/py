...
CJK_FONT_FAMILY = 'Noto Sans TC'
CJK_FONT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'fonts', 'NotoSansTC-VF.ttf')


def register_cjk_font(path=CJK_FONT_PATH):
  """Register the bundled CJK font with fontconfig so Pango can find it
  even when no system-wide CJK font (e.g. Noto Sans CJK TC) is installed."""
  if not os.path.exists(path):
    logging.warning('CJK font not found: %s', path)
    return False
  try:
    ...
    if not fc.FcConfigAppFontAddFile(cfg, path.encode('utf-8')):
      logging.warning('failed to register CJK font: %s', path)
      return False
    return True
  except OSError as e:
    logging.warning('failed to load libfontconfig: %s', e)
    return False
...
class Demo:
  ...
  def on_caps_changed(self, overlay, caps):
    ...
    logging.info('camera caps: %dx%d', w, h)
  ...
  def on_draw(self, overlay, context, timestamp, duration):
    ...
    font_desc = Pango.FontDescription(f'{CJK_FONT_FAMILY} Bold 12')
