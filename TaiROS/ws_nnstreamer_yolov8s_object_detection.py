...
class Demo:
  def __init__(self, argv=None):
    ...
    self.ws_url = 'ws://192.168.60.1:8011/ws/camera'
  ...
  def _src(self):
    ...
    elif self.cam_type == 'websocket':
      return (f'appsrc name=src is-live=true format=time block=true '
  ...
  def build_pipeline(self):
    ...
    if self.cam_type in ('uvc', 'websocket'):
  ...
  # ---- WebSocket thread: receive JPEG frames and push into appsrc -----------
  def _ws_thread(self):
    src = self.pipeline.get_by_name('src')
    self._ws_start_time = time.monotonic()

    async def _recv():
      logging.info('[ws] connecting to %s', self.ws_url)
